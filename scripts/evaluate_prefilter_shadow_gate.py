"""Read-only Phase 2 §6.4 evaluator-prefilter shadow gate.

The command never initializes or migrates the database, never calls an LLM or
embedding provider, and never changes ``eval_prefilter_mode``.  It snapshots a
maximum audit id inside one read transaction so the evaluated cohort is stable
even while the daemon appends newer shadow rows.
"""
...
from openbiliclaw.discovery.prefilter_audit import evaluate_prefilter_gate  # noqa: E402

_TABLE = "evaluator_prefilter_shadow_audit"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate the read-only Phase 2 prefilter shadow quality gate.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Exclude rows at or below this privacy-safe audit id.",
    )
    parser.add_argument(
        "--through-id",
        type=int,
        default=None,
        help="Optional inclusive frozen upper audit id; defaults to the current maximum.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = evaluate_prefilter_gate(rows).to_dict()
    report["cohort"] = {
