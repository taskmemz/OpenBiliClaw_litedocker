...
class YtTaskQueue:
    ...
    def enqueue_with_id(
        ...
        *,
        ...
        if daily_budget > 0 and count_today >= daily_budget:
            logger.info(
                "yt task budget exhausted: type=%s used_today=%d budget=%d "
                "(per-day UTC cap from config [sources.youtube] daily_*_budget; "
                "0 = unlimited)",
                task_type,
                count_today,
                daily_budget,
    ...
    def stage_final_result(
        ...
        *,
        ...
        def merge(current: dict[str, Any]) -> dict[str, Any]:
            merged, _added = _merge_yt_result_payload(
                current,
                items=items,
                scope_counts=scope_counts,
                debug=debug,
    ...
    def complete_staged_result(self, task_id: str) -> bool:
        ...
        return complete_staged_result(self._db, table="yt_tasks", task_id=task_id)
